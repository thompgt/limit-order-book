...
| | |
|---|---|
| `POST /api/v1/orders` | submit. `{symbol, side, type, timeInForce, price, quantity, orderId?}` |
...
| **Java** | Java 21, no framework and no Lombok in the core: intrusive doubly-linked lists, an ownership contract on recycled objects, sealed-off package-private mutation, and a test suite that names the semantics it pins — 241 tests, 22 of them property-based with jqwik |
...
WebSocket feed and book viewer. 241 tests green — 201 in `engine-core`, 40 in
